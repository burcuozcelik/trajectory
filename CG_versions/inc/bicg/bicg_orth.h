...
BiCG(const Matrix &A, Vector &x, const Vector &b,
     const Preconditioner &M, int &max_iter, Real &tol, int optIteration, double thr)
{
  Real resid;
  ...
  /*GK: Error detection, Check SDC, orthogonality Check*/
  double ex1, ex2;
  /*****************/
  ...
  for (int i = 1; i <= max_iter; i++) {
    ...
    /*GK: Error detection, Check SDC, orthogonality Check*/
    /*****************/
    if ((i > 1) && ((i % optIteration) == 0)){
      ex1= dot(ptilde,q) / (norm(ptilde) * norm(qtilde));
      ex2 = norm(r + A*x -b) / (norm(b) * normM(A));
      dCheck(thr, ex1, ex2, i);
    }
    /*****************/
